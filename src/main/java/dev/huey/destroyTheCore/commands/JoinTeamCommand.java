...
public class JoinTeamCommand extends Subcommand {
  public JoinTeamCommand() {
    super("team");
    addArgument("team", () ->
      Arrays.stream(Game.Side.values())
        .map(s -> s.id).toList()
    );
    addArgument("player", () ->
  ...
  public void execute(Player pl, List<String> args) {
    ...
    Game.Side side = Arrays.stream(Game.Side.values())
      .filter(s -> s.id.equals(args.getFirst()))
      .findAny().orElse(null);
    if (side == null) {
    ...
    if (args.size() >= 2) {
      ...
      if (target == null) {
        PlayerUtils.prefixedSend(pl, TextUtils.$("commands.join.player-not-found"));
        return;
      }
      PlayerUtils.prefixedBroadcast(TextUtils.$("commands.join.made-other", List.of(
      ...
      target = pl;
      PlayerUtils.prefixedBroadcast(TextUtils.$("commands.join.made-self", List.of(
        Placeholder.component("player", PlayerUtils.getName(pl)),
        Placeholder.component("side", side.titleComp())
      )));
    }
