...
public class LotteryGen extends UsableItemGen {
  ...
  void give(Player pl, ItemStack item, boolean big) {
    ...
    item.editMeta((meta) -> {
      if (
        ...
        DestroyTheCore.itemsManager.gens.get(ItemsManager.ItemKey.PLACEHOLDER).checkItem(item)
      ) return;
    ...
    announcer.accept(TextUtils.$("items.lottery.announce.item", List.of(
      Placeholder.component("player", PlayerUtils.getName(pl)),
      Placeholder.component(
        "item",
        item.effectiveName()
          .colorIfAbsent(
            item.getItemMeta().hasRarity()
              ? item.getItemMeta().getRarity().color()
  ...
  void giveTreasure(Player pl) {    
    ...
    pl.give(DestroyTheCore.itemsManager.gens.get(ItemsManager.ItemKey.RANDOM_ROLE).getItem());
  }
  ...
  List<Pair<Integer, Consumer<Player>>> junk = List.of(
    Pair.of(
      10,
      (pl) -> give(pl, new ItemStack(Material.CAKE))
    ),
    Pair.of(
      10,
      (pl) -> give(pl, new ItemStack(Material.EXPERIENCE_BOTTLE, 3))
    ),
    Pair.of(
      10,
      (pl) -> give(pl, new ItemStack(Material.COOKED_CHICKEN))
    ),
    Pair.of(
      10,
      (pl) -> give(pl, new ItemStack(Material.LEATHER))
    ),
    Pair.of(
      10,
      (pl) -> give(pl, new ItemStack(Material.TURTLE_EGG))
    ),
    Pair.of(
      10,
      (pl) -> give(pl, new ItemStack(Material.ARROW, 3))
    ),
    Pair.of(
      10,
      (pl) -> give(pl, new ItemStack(Material.BONE_MEAL, 3))
    ),
    Pair.of(
      10,
      (pl) -> give(pl, new ItemStack(Material.PUMPKIN_PIE))
    ),
    Pair.of(
      10,
      (pl) -> give(pl, new ItemStack(Material.BEETROOT_SOUP))
    ),
    Pair.of(
      10,
    ...
    Pair.of(
      ...
      (pl) -> {
        ...
        announce(TextUtils.$("items.lottery.announce.poison", List.of(
          Placeholder.component("player", PlayerUtils.getName(pl)),
          Placeholder.component("side",
    ...
    Pair.of(
      ...
      (pl) -> {
        ...
        announce(TextUtils.$("items.lottery.announce.wear", List.of(
          Placeholder.component("player", PlayerUtils.getName(pl)),
    ...
    Pair.of(
      25,
      (pl) -> {
        teamAnnounce(pl, TextUtils.$("items.lottery.announce.nothing", List.of(
          Placeholder.component("player", PlayerUtils.getName(pl))
        )));
      }
    ),
    Pair.of(
      ...
      (pl) -> {
        for (int i = 0; i < 9; ++i)
          pl.getInventory().setItem(i, new ItemStack(Material.ROTTEN_FLESH));
        
        announce(TextUtils.$("items.lottery.announce.replace-hotbar", List.of(
          Placeholder.component("player", PlayerUtils.getName(pl))
        )));
      }
    ),
    Pair.of(
      ...
      (pl) -> {
        ...
        DestroyTheCore.game.getPlayerData(pl)
          .addRespawnTime(-PlayerData.killPunishment * teammates.size());
          
        for (Player p : teammates) {
          if (!PlayerUtils.shouldHandle(p)) continue;
          
          DestroyTheCore.damageManager.addDamage(pl, p, Double.MAX_VALUE);
          p.damage(Double.MAX_VALUE, DamageSource.builder(DamageType.MAGIC).build());
        }
        
        CoreUtils.setTickOut(() -> {
          announce(TextUtils.$("items.lottery.announce.kill", List.of(
            Placeholder.component("player", PlayerUtils.getName(pl)),
            Placeholder.unparsed(
              "action",
              RandomUtils.pick(
                TextUtils.translateRaw("items.lottery.announce.kill-actions")
                  .split("\\|")
              )
            ),
            Placeholder.component("side", DestroyTheCore.game.getPlayerData(pl).side.titleComp())
          )));
        }, 2); // Make sure this message comes after the kill messages
      }
    ),
    Pair.of(
      5,
      (pl) -> {
        ...
        CoreUtils.setTickOut(() -> {
          announce(TextUtils.$("items.lottery.announce.send-spawn", List.of(
            Placeholder.component("player", PlayerUtils.getName(pl)),
            Placeholder.component("side", DestroyTheCore.game.getPlayerData(pl).side.titleComp())
          )));
        }, 2);
    ...
    Pair.of(
      ...
      (pl) -> {
        ...
        announce(TextUtils.$("items.lottery.announce.chaos", List.of(
          Placeholder.component("player", PlayerUtils.getName(pl))
        )));
      }
    )
