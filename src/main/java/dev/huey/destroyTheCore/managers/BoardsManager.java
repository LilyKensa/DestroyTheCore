...
public class BoardsManager {
  ...
  public void refresh(Player pl) {
    ...
    lines.addAll(List.of(
      "",
      TextUtils.$r("board.team", List.of(
        Placeholder.component("side", data.side.titleComp())
      ))
    ));
    
    if (inGame) {
      lines.add(TextUtils.$r("board.role", List.of(
        Placeholder.unparsed("role", data.role.name)
      )));
    }
    ...
    if (DestroyTheCore.game.isPlaying) {
      ...
      lines.addAll(List.of(
        "",
        TextUtils.$r("board.phase", List.of(
        ...
        TextUtils.$r("board.countdown", List.of(
          Placeholder.unparsed("time", CoreUtils.formatTime(
            Math.ceilDiv(DestroyTheCore.game.phaseTimer, 20),
            "§e"
          ))
        )),
        "",
        TextUtils.$r("board.health", List.of(
          Placeholder.unparsed("a", col1 + side1.coreHealth + coreInvulnDisplay.apply(side1)),
          Placeholder.unparsed("b", col2 + side2.coreHealth + coreInvulnDisplay.apply(side2))
        ))
      ));
      
      if (DestroyTheCore.game.isInTruce()) {
        lines.add(
          TextUtils.$r("board.truce", List.of(
            Placeholder.unparsed("time", CoreUtils.formatTime(
      ...
      lines.addAll(List.of(
        "",
        TextUtils.$r("board.wins", List.of(
          ...
          Placeholder.unparsed(
            ...
            stats.games == 0
              ? "0.0"
              : CoreUtils.toFixed(100D * stats.wins / stats.games)
          )
        )),
        TextUtils.$r("board.kd", List.of(
        ...
        TextUtils.$r("board.ores", List.of(
          Placeholder.component(
            "value",
            Component.text(stats.ores.values().stream().reduce(0, Integer::sum))
          )
        )),
        ""
      ));
